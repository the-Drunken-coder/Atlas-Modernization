...
The server fixture configures only the disposable loopback Core and clears deployed-target configuration. The test verifies that both the server and browser expose one non-deployed target, keep the API-key input empty, do not show deployed confirmation, and do not disclose the configured Core API key in browser diagnostics. Its isolated package root starts with an empty cleanup ledger, so a developer's worktree ledger is preserved and ignored. The shared acceptance runner assigns a UUID Compose project with unique credentials, ports, containers, volumes, and labels, then removes only that project.
