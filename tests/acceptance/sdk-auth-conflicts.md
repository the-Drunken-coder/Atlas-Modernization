...
The scenario owns an isolated disposable Compose project and writes `run.json`, `commands.log`, `evidence.jsonl`, `stack.json`, `compose.log`, and `result.json` below `.atlas/acceptance/sdk-auth-conflicts/<run-id>/`. The generated API, admin, PostgreSQL, and MinIO credentials are never recorded in evidence or command diagnostics. The required CI execution is `.github/workflows/sdk-auth-conflicts.yml`, which uploads the same evidence directory for every run and fails when that directory is absent.
