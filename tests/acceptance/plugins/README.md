...
The command builds the actual Core development image, Source Gateway production target, Reference Plugin target, and Reference source target. The source target mounts a test-only deterministic fixture on an internal network that only Source Gateway joins; the Plugin stays on the default network and the scenario proves it cannot fetch the source directly. It makes no public provider request and does not use the fixed container names or network from the development Plugin Compose file.
