...
The required journey verifies the runtime manifest and its declared `search_buildings` Operation. It sends a fixed map area and checks the returned feature, geometry, attribution, provenance, and a retrieval timestamp within the invocation. It also checks invalid map-area input, malformed source geometry, a controlled source outage, and caller cancellation after the slow request reaches the local source and closes its connection. The Plugin must remain available after that cancellation. Plugin unavailability and recovery after the same owned container restarts are covered too. Error assertions require their documented details in the SDK response and use a cloned raw Core response for the required `error_id`, timestamp, and route metadata that the SDK does not retain.
