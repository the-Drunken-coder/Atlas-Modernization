...
The required journey verifies the runtime manifest and its declared `search_buildings` Operation. It sends a fixed map area and checks the returned feature, geometry, attribution, provenance, and a retrieval timestamp within the invocation. It also checks invalid map-area input, malformed source geometry, a controlled source outage, and caller cancellation after the slow request reaches the local source and closes its connection. Plugin unavailability and recovery after the same owned container restarts are covered too. Error assertions inspect the SDK payload and a cloned raw Core response. Core may omit error route, identity, timestamp, and details metadata, so those fields are checked when present.
