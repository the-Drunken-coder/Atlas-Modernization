...
Acceptance scenarios use `support/stack.mjs` to run the guarded acceptance Core image against run-owned, disposable PostgreSQL and MinIO storage. The acceptance image retains Go overlays for test fixtures but applies the production entrypoint's durable-storage and credential guards. Durable mode preserves state for an in-run Core restart; cleanup destroys only those generated volumes. The runner owns:
