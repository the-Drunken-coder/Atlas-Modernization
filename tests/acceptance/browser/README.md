...
At historical pre-rebase revision `06472c8626560d0834197024f163fa1d9dc92fe3`, whose equivalent published source is `27d1c367cd8e62e6e228af89c2a17d94c1a737ea`, the corrected Chromium journey passed creation, real map selection, vertex editing, stale-save draft retention, discard confirmation, drawing precedence, save, reload, and independent Core-read checks. It then failed the active deletion assertion because the selected Geofeature inspector exposed `Edit` and `Raw entity JSON` but no deletion control. The 88.968-second first failure remains under `.atlas/acceptance/browser-geofeatures-chromium/browser-geofeatures-chromium-resume-380-06472c86-8b971b52-d3ec-4e4c-a7db-b04ff50c16ef/`. The command interface now provides the tested deletion control, and the resolved defect record was retired with that repair.
