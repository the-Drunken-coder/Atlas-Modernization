...
The committed development configuration enables only the deterministic `reference` connector and explicitly permits the private Compose address. The production configuration contains no connectors. Operators add production connectors and secret references through deployment-managed configuration; credentials do not belong in this file or Atlas resources.
