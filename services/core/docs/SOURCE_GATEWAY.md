...
Caching and retries are disabled when their zero values are used. A cached route must be read-only. `429` and `5xx` responses are never cached. Retrying a mutating route requires `idempotency_header`, and each request must supply it. Retry failures may contain only `upstream_timeout` and `upstream_unreachable`. The connector timeout bounds admission and every configured retry. Expiry while waiting for concurrency or rate-limit admission returns `admission_timeout`; expiry before the Gateway buffers the complete bounded upstream response returns `upstream_timeout`. Once buffered, later local header filtering, encoding, or response cleanup cannot turn a valid response into a timeout. A valid upstream HTTP response, including `3xx`, `4xx`, or `5xx`, is returned to the Plugin and resets the connector circuit.
