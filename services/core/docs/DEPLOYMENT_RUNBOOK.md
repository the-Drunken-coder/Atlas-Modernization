...
This uses `services/core/docker/docker-compose.production.yml`, builds the
...
python3 services/core/scripts/atlas.py --production --tunnel
```
...
migration_table_present="$(
  docker compose -f services/core/docker/docker-compose.production.yml exec -T \
...
(
  set -e
  docker compose -f services/core/docker/docker-compose.production.yml stop api
) || exit "$?"
```
...
(
  ...
  docker compose -f services/core/docker/docker-compose.production.yml exec -T postgres \
...
./services/core/scripts/run_integration_tests.sh
```
...
```bash
docker compose -f services/core/docker/docker-compose.production.yml logs -f api postgres minio
```

Production tunnel logs:

```bash
docker compose -f services/core/docker/docker-compose.production.yml \
  -f services/core/docker/docker-compose.tunnel.yml logs -f api cloudflared
```

Stop containers without deleting volumes:

```bash
docker compose -f services/core/docker/docker-compose.production.yml down --remove-orphans
```
