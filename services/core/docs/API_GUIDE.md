...
The canonical resource and wire schemas live in Atlas Protocol. Use this guide for route behavior, then use `../../../packages/protocol/examples/` and `../../../packages/protocol/schema/jsonschema/atlas.schema.json` for exact payload examples and schema details.
...
Protected Core routes accept the Core-owned browser session cookie. Local browser development uses the seeded admin session. The default `atlas.py --dev` launcher enables API-key auth and stores its generated machine key in the owner-only `services/core/docker/.env.local`; local server-side clients can use that bootstrap key. Manually configured machine clients should set `ENABLE_API_AUTH=true` and send either the bootstrap `API_AUTH_KEY` or an active managed API key with one of:
...
LOGIN_JSON="$(
  python3 - <<'PY'
import json
from services.core.scripts.compose_env import parse_compose_env_file

password = parse_compose_env_file("services/core/docker/.env.local").get("ATLAS_ADMIN_PASSWORD")
if not password:
    raise SystemExit("services/core/docker/.env.local must contain ATLAS_ADMIN_PASSWORD")
