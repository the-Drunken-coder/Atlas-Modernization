...
3. From the **repository root**, start the development tunnel stack:
   ...
   ```bash
   python3 services/core/scripts/atlas.py --tunnel
   ```

4. For the production-image stack, load the complete operator-owned production
   environment described in `services/core/docs/DEPLOYMENT_RUNBOOK.md`, including
   ...
   `services/core/docker/.env`, then start from the repository root:
...
- `atlas.py --tunnel` may load `CLOUDFLARE_TUNNEL_TOKEN`, `API_AUTH_KEY`, and
  `ATLAS_ADMIN_PASSWORD` from the shell or `services/core/docker/.env`.
...
- For development `atlas.py --tunnel`, verify the token, API key, and admin
  ...
  `services/core/docker/.env`. For `atlas.py --production --tunnel`, export the
  required Core credentials; the tunnel token may be exported or stored in
  `services/core/docker/.env`.
- Check tunnel logs: `cd services/core/docker && docker compose -f docker-compose.yml -f docker-compose.tunnel.yml logs cloudflared`
