...
def resolve_atlas_core_dir():
    """Return the absolute path to the services/core directory."""
    current_dir = os.path.abspath(os.path.dirname(__file__))
    search_dir = current_dir
    while True:
        if os.path.basename(search_dir) == "core" and os.path.basename(os.path.dirname(search_dir)) == "services":
            return search_dir
        candidate = os.path.join(search_dir, "services", "core")
        ...
        if parent == search_dir:
            break
        search_dir = parent
    raise FileNotFoundError("services/core directory not found")
...
def raise_for_exited_development_core(container_name="atlas_core_api"):
    ...
    if (
        ...
        raise RuntimeError(
            ...
            "or, if its data is disposable, rerun with: python3 services/core/scripts/atlas.py "
    ...
    try:
        ...
        if not db_only:
            ...
            if not production and not tunnel:
                print("  Admin:     admin (password stored in services/core/docker/.env.local)")
            if production:
