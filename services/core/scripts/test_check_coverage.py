...
def block(group: str, statements: int, count: int) -> str:
    return f"github.com/example/atlas/services/core/internal/{group}/sample.go:1,2 {statements} {count}"
...
def baseline_lines(
    ...
    return lines + [
        f"github.com/example/atlas/services/core/other.go:1,2 {other_covered} 1",
        f"github.com/example/atlas/services/core/other_uncovered.go:1,2 {other_total - other_covered} 0",
    ]
            ...
            path.write_text(
                profile(
                    [
                        ...
                        block("storage", 3, 1),
                        block("admin", 2, 1),
                        "github.com/example/atlas/services/core/other.go:1,2 7 0",
                    ]
                ),
                encoding="utf-8",
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            path.write_text(
                profile(
                    [
                        "malformed",
                        "github.com/example/atlas/services/core/internal/actions/bad.go:1,2 1",
                        "github.com/example/atlas/services/core/internal/actions/bad.go:1,2 nope 1",
                        "github.com/example/atlas/services/core/internal/actions/bad.go:1,2 1 nope",
                        block("actions", 2, 1),
                    ]
                ),
                encoding="utf-8",
