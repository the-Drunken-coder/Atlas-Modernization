...
func (a *TaskActions) Create(ctx context.Context, params CreateTaskParams, idempotencyKey string) (*models.Task, bool, error) {
	...
	if err := rejectGotoDuringTakeoff(ctx, tx, params.AssetID, runtimeID, params.Command); err != nil {
		return nil, false, err
	}
...
func rejectGotoDuringTakeoff(ctx context.Context, tx pgx.Tx, assetID, runtimeID, command string) error {
	if command != "flight.goto" {
		return nil
	}
	var takeoffID string
	err := tx.QueryRow(ctx, `SELECT task_id FROM tasks
		WHERE asset_id = $1 AND runtime_id = $2 AND command = 'flight.takeoff'
		AND status IN ('pending', 'acknowledged', 'in_progress')
		ORDER BY created_at, task_id FOR UPDATE LIMIT 1`,
		assetID, runtimeID).Scan(&takeoffID)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil
	}
	if err != nil {
		return fmt.Errorf("lock active takeoff for go-to: %w", err)
	}
	return NewValidationError("Go-to cannot interrupt an active takeoff")
}
