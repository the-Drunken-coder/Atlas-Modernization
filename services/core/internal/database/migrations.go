...
const (
	...
	resourceInstanceTokensMigrationName     = "immutable_resource_instance_tokens"
	resourceInstanceTokensMigrationChecksum = "04dfff48b1f7db7bea2da4193c60da87216dc0e783b861b0413ff275b8c77658"
...
func coreSchemaMigrations() []schemaMigration {
	return []schemaMigration{
		...
		{
			version:            9,
			name:               resourceInstanceTokensMigrationName,
			checksum:           resourceInstanceTokensMigrationChecksum,
			fingerprintVersion: fingerprintVersionV2,
			statements: []string{
				`CREATE TABLE resource_instance_tokens (
					token_hash VARCHAR(64) PRIMARY KEY,
					created_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
					CONSTRAINT resource_instance_tokens_hash_length CHECK (length(token_hash) = 64)
				)`,
				`ALTER TABLE entities ADD COLUMN instance_token_hash VARCHAR(64)
					CHECK (instance_token_hash IS NULL OR length(instance_token_hash) = 64)`,
				`ALTER TABLE objects ADD COLUMN instance_token_hash VARCHAR(64)
					CHECK (instance_token_hash IS NULL OR length(instance_token_hash) = 64)`,
			},
		},
	}
