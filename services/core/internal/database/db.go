...
var coreSchemaTables = []string{"entities", "tasks", "asset_runtime_generations", "asset_runtimes", "objects", "atlas_change_clock", "atlas_change_events", "object_deletion_fences", "storage_deletion_outbox", "storage_upload_intents", "admin_records"}
...
func scratchDataResetDDL() []string {
	return []string{
		`TRUNCATE TABLE atlas_change_events, object_deletion_fences, storage_upload_intents, storage_deletion_outbox, tasks, asset_runtimes, asset_runtime_generations, entities, objects RESTART IDENTITY CASCADE`,
