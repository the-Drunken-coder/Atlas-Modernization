...
func scratchDataResetDDL() []string {
	return []string{
		`TRUNCATE TABLE entity_movement_samples, atlas_change_events, object_deletion_fences, resource_instance_tokens, storage_upload_intents, storage_deletion_outbox, tasks, asset_runtimes, asset_runtime_generations, entities, objects RESTART IDENTITY CASCADE`,
