...
func typeScriptSource(revision string, schemas map[string][]byte) ([]byte, error) {
	if _, err := validateProtocolRevision(revision); err != nil {
		return nil, err
	}
	...
	builder.WriteString("import { ATLAS_PROTOCOL_REVISION } from \"./revision.js\";\n\n")
	builder.WriteString("export { ATLAS_PROTOCOL_REVISION };\n")
...
func typeScriptRevisionSource(revision string) ([]byte, error) {
	revision, err := validateProtocolRevision(revision)
	if err != nil {
		return nil, err
	}
	return formatTypeScript([]byte("// Code generated by go run ./tools/generate; DO NOT EDIT.\n\nexport const ATLAS_PROTOCOL_REVISION = " + jsonString(revision) + " as const;\n")), nil
}
