...
The repository carries a temporary patch for `@meshtastic/core` 2.6.7 that cancels each packet queue timeout when its entry is acknowledged, rejected, removed, or cleared during disconnect. The compiled-process acceptance verifies that no referenced queue timeout remains after shutdown and that both Link processes exit normally within 15 seconds. Remove the patch after an upgraded dependency includes the same lifecycle cleanup.
