...
The same compiled-process run keeps three paths isolated. A mismatched Gateway membership must reject startup and release its listener and transport. A Gateway and Asset using different authentication keys must exchange a public-channel challenge while the Asset remains in bounded discovery. The joined pair submits two confirmed subscription operations. The public Asset event and operation routes show that the radio accepted each packet while its result remained `queued`; the Gateway application then settles one as `confirmed` and the other as `rejected`, and both services report no pending transport work. The controller finally signals every process, verifies lifecycle-owned cleanup, waits for the joined Gateway event stream to close, and requires normal exits.
