...
The acceptance starts independent Gateway and Asset Node processes through the public `startLinkService` lifecycle and `MeshtasticSerialRadio.openTransport`. A test-owned device protocol fixture drives real profile configuration, commit, reconnect, readback, Gateway membership activation, and pre-shared-key authenticated joining. The controller observes public loopback status and Radio profile responses, then verifies an ordinary Asset state publication through both the Gateway event stream and Shared Picture.

The same compiled-process run keeps three paths isolated. A mismatched Gateway membership must reject startup and release its listener and transport. A Gateway and Asset using different authentication keys must exchange a public-channel challenge while the Asset remains in bounded discovery. The joined pair submits confirmed subscription operations. The public Asset event and operation routes show that the radio accepted each packet while its result remained `queued`; the Gateway application then settles one as `confirmed` and another as `rejected`, and both services report no pending transport work. A final operation remains unsettled until shutdown, with public status proving one pending confirmation on the Asset and one pending inbound settlement on the Gateway. The focused `LinkService.stop` test verifies that pending operation results synchronously become `failed` with `link service stopped`; the process run verifies that the shared lifecycle completes that stop and releases its HTTP listeners, radio connections, and pending writes. The HTTP shutdown contract closes active event streams but does not guarantee delivery of a final SSE record.

Set `ATLAS_LINK_ACCEPTANCE_ARTIFACTS` to select the artifact parent directory. Each run preserves expected and observed JSON, the activated membership records, child summaries, and separate child stdout and stderr logs under a UUID-owned scenario directory. `ATLAS_LINK_ACCEPTANCE_SEED` selects the recorded laboratory node numbers, private-channel key byte, and authentication-key labels. It does not seed Link service sessions, join and transport identifiers, cryptographic randomness, or operating-system scheduling. Pull requests and pushes use seed `390`. The scheduled nightly workflow runs the required paths with seed `39001`, then uses seed `39002` to drop every frame in one joined Asset private-channel attempt. The controller decodes those test frames to keep delivery disabled through the complete first attempt, observes radio acceptance, a queued operation, and no Gateway settlement, then restores delivery and requires the Link retry to reach application confirmation. It continues with the ordinary confirmation, rejection, publication, pending-state, and shutdown checks. This controlled fault does not exercise partial-frame loss or reassembly expiry. All runs preserve the `SHORT_FAST`, channel 20, `LOCAL_ONLY` profile and upload evidence even when a scenario fails.

The pinned [`@meshtastic/core` 2.6.7 shutdown defect](../../docs/problems/2026-09-12-meshtastic-queue-timers-delay-process-exit.md) currently prevents the shutdown check from passing. Its write queue schedules a referenced 60-second timeout for every queued packet, but packet acknowledgement and disconnect clear queue entries without cancelling those timers. The acceptance records zero lifecycle-owned connections and pending writes after `startLinkService.close()`, then records the remaining Node resource types after IPC disconnect. Keep the 15-second normal-exit assertion active for a separate dependency-lifecycle repair; a fallback signal is cleanup evidence, not a successful Link process exit.

Set `ATLAS_LINK_ACCEPTANCE_FAULT=drop-asset-private-packets` to exercise the same bounded private-delivery recovery path locally. This is the only supported fault value; there is no seeded random network model in this acceptance.
