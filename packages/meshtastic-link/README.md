...
Set `ATLAS_LINK_ACCEPTANCE_ARTIFACTS` to select the artifact parent directory. Each run preserves expected and observed JSON, the activated membership records, child summaries, and separate child stdout and stderr logs under a UUID-owned scenario directory. `ATLAS_LINK_ACCEPTANCE_SEED` selects the recorded laboratory node numbers, private-channel key byte, and authentication-key labels. It does not seed Link service sessions, join and transport identifiers, cryptographic randomness, or operating-system scheduling. Pull requests and pushes use seed `390`. The scheduled nightly workflow runs the required paths with seed `39001`, then uses seed `39002` to drop the joined Asset's private-channel packets until the controller observes radio acceptance, a queued operation, and no Gateway settlement. It restores delivery and requires the Link retry to reach application confirmation before continuing the ordinary confirmation, rejection, publication, pending-state, and shutdown checks. All runs preserve the `SHORT_FAST`, channel 20, `LOCAL_ONLY` profile and upload evidence even when a scenario fails.
