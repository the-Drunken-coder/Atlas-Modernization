...
The pinned `@meshtastic/core` 2.6.7 currently prevents the shutdown check from passing. Its write queue schedules a referenced 60-second timeout for every queued packet, but packet acknowledgement and disconnect clear queue entries without cancelling those timers. The acceptance records zero lifecycle-owned connections and pending writes after `startLinkService.close()`, then records the remaining Node resource types after IPC disconnect. Keep the 15-second normal-exit assertion active for a separate dependency-lifecycle repair; a fallback signal is cleanup evidence, not a successful Link process exit.
