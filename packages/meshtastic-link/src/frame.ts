...
export function fragmentPayload(
  ...
  let low = 1;
  let high = payload.byteLength;
  let frames: Uint8Array[] | undefined;
  while (low <= high) {
    const chunkSize = Math.floor((low + high) / 2);
    const candidate = framesForChunkSize(payload, identity, chunkSize);
    if (candidate.length <= MAX_FRAGMENTS && candidate.every((frame) => frame.byteLength <= maxFrameBytes)) {
      frames = candidate;
      low = chunkSize + 1;
    } else {
      high = chunkSize - 1;
  ...
  if (frames === undefined) throw new RangeError("Link envelope leaves no room for a payload chunk");
  return frames;
}
...
function encodeNode(node: LinkNode): string {
  if (!node.id || node.id.includes(":")) throw new TypeError("Link node IDs must be non-empty and cannot contain ':'");
...
function decodeNode(value: string): LinkNode {
  ...
  if ((role !== "a" && role !== "g") || !id || id.includes(":")) throw new TypeError("Invalid Link node identity");
...
function isMessageType(value: unknown): value is MessageTypeCode {
  return ["s", "t", "p", "q", "r", "o", "u", "b", "c"].includes(String(value));
}

function isPriority(value: unknown): value is PriorityCode {
  return ["s", "t", "q", "l", "r", "o"].includes(String(value));
}
