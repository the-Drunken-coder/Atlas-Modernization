...
export function fragmentPayload(
  ...
  for (
    ...
    if (count > MAX_FRAGMENTS || !framesFit(payload, identity, chunkSize, count, maxFrameBytes, encoding)) continue;
  ...
  if (compressed !== undefined) {
    try {
      for (let chunkSize = Math.min(compressed.byteLength, maxFrameBytes); chunkSize > 0; chunkSize--) {
        ...
        if (count > MAX_FRAGMENTS || !framesFit(compressed, identity, chunkSize, count, maxFrameBytes, encoding))
          continue;
