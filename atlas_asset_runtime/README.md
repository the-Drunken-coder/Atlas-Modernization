...
Queued Commands use one serial executor. Immediate Commands start independently. A terminal Task update from Core aborts the matching handler through its `AbortSignal`, including cancellation and runtime fencing. Throw `AssetTaskFailure("precondition_failed", message)` when a physical or operational precondition prevents execution. Other handler errors become `execution_failed`. Check-in reports telemetry and observed state only. It never carries or retrieves Tasks.
