...
The Link retains up to 4,096 distinct outbound confirmed operation identities while they are pending and for ten minutes after terminal completion. This retention horizon is twice the longest five-minute retry deadline, so a retry can continue to use its admitted identity without making the session's identity memory unbounded. Pending identities remain reserved until they reach a terminal outcome; retention expiry only releases completed identity capacity and does not delete the ordinary operation result early. If all identity slots are occupied, the Link rejects a new confirmed identity explicitly and wakes blocked capacity waiters when the earliest completed identity expires.

A separate inbound budget reserves up to 4,096 pending or completed settlements. Pending settlements keep their reservations; completed settlements expire after ten minutes. Capacity rejection cannot consume a slot reserved for an admitted operation. Normal renewals therefore release identity capacity in both directions.

After the retention horizon, clients must use a fresh operation identity for a new operation. The Link does not guarantee that reusing an expired identity is safe, because the receiver's settled-inbound history has its own bounded retention. Durable Task IDs and Atlas Core reconciliation remain responsible for recovering semantic outcomes beyond the Link's retention horizon. Restarting and rejoining also creates a new service session and a new identity fence.

## Bounded failure

Confirmed operations use stable request or operation identities. The receiver suppresses repeated application delivery while its bounded settled-inbound history retains the settlement, and the sender keeps an admitted identity fenced while it is pending and for ten minutes after terminal completion. These histories have separate bounds, so the Link does not guarantee duplicate suppression after the documented retention horizon. Retries are bounded. Exhaustion produces a visible failure for the originating Link client and bounded diagnostic evidence. The operation never becomes confirmed merely because retrying stopped.
...
A service admits at most 4,096 distinct source Link identities across transport and Shared Picture. At capacity it refuses new identities while continuing to accept current sources and their newer generations. Source generation fences remain for the service session; they are not evicted to make room. Picture admission must succeed before transport commits a new source fence. Gateway activation reports a typed source-admission error before join acceptance when this admission fails.

Within the active generation and service session, increasing source sequence rejects duplicate and reordered stale updates to the same record. Per-record sequence fences expire after ten minutes so this replay state remains bounded; observation time stays visible if an older best-effort publication arrives after that window. Confirmed operations use their stable request, Task, or operation identity for pending and bounded-horizon transport deduplication and application-level reconciliation. The sender and receiver retain duplicate history independently, so clients use fresh identities after the documented retention horizon.
