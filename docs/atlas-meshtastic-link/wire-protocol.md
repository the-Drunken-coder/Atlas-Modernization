...
The `message-v2` profile can also emit every fallback representation understood by `message-v1`. Receipts retain the same addressed, single-frame Task-report restriction. Deploy compatible software across the fleet before enabling it; joining does not negotiate this profile. See [compression method comparison](../../packages/meshtastic-link/experiments/COMPRESSION-METHODS.md) for measured costs and benefits.
