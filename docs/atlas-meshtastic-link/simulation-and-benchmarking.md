...
The stress workload runs the complete normal workload and adds twenty distinct active Tracks at one update per second (1,200 additional publications over the window), one 32 KiB Object transfer, and an urgent cancellation while background traffic is queued. Both scenarios use the same fleet runner and application handlers. Two subscriptions are submitted in a quiet setup phase, spaced thirty seconds apart. Normal measurement starts after setup; stress first requests Object content through the radio and starts measurement when the Gateway sends its first content chunk. Cancellation is submitted 500 ms later.
...
The checked-in seed-42 position, canonical five-radio normal, and canonical stress baseline results live in `packages/meshtastic-link/baselines`. The canonical normal scenario is revision 4 and the stress scenario is revision 3 because both now record the complete normal workload, the stress additions, an explicit three-hop simulator limit, and a finite 60-second measurement window. Every result includes a self-describing scenario manifest and SHA-256 hash covering the radio profile, regulatory region, topology, network assumptions, production path, workload rates, payload fixture revision, transport assumptions, and success criteria. Package tests rerun all three through the production transport and fail if their semantics or exact measurements drift without an intentional baseline update. The canonical picture snapshots preserve the exact 30-second, 60-second, and final post-drain observations; stress records its 60-second measurement and separate post-drain snapshot. Each accepted add or renewal schedules expiry ninety seconds after actual receipt, including during setup and drain. Feed metrics capture actual active listeners and unexpired subscriber leases at the sixty-second observation and after drain. A late renewal may reopen an expired feed. Seed 42 records zero picture records at thirty seconds and one at sixty seconds in the normal scenario; stress also records one at sixty seconds. Neither scenario completes its normal Task or request workload, and stress completes neither cancellation nor Object transfer.
...
Meshtastic Lab orchestration, native-firmware workloads, comparison scripts, and experiment reports are maintained outside Atlas. Atlas retains the resulting transport improvements and focused tests, plus the deterministic packet benchmarks described above.

Packet drops, complete-message acceptance, sender confirmation, and missed deadlines are separate outcomes. Recovered packet loss adds delivery cost; a lost confirmation does not erase receiver acceptance. Native firmware simulations and physical-radio observations must retain separate provenance.

The packet model carries received native packet IDs, applies the per-send native payload budget, and includes request metadata in estimated airtime. It does not model firmware queue priority and eviction. These limitations remain relevant when comparing benchmark results with external experiments.
