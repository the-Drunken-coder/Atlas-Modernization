...
Current Entity writes capture samples in their existing transaction. Imports acquire the same Entity row lock and allocate sequences only afterward. They validate and normalize inputs before locking, then insert the eligible batch with one database statement. Duplicate comparison covers both stored reports and repeated IDs within the entire batch, including expired inputs; any conflict rolls back the whole batch. Import eligibility uses one retention cutoff calculated after acquiring the lock, so time spent waiting for another writer does not keep expired reports eligible. They never modify the Entity, heartbeat, resource version or feed clock. This ordering makes the committed per-Entity sequence a usable pagination boundary. Concurrent current writes and imports are covered by a PostgreSQL test that observes both blocked writers before releasing the row lock.
...
Movement History sits immediately after Location & Movement in the existing Asset/Track sidebar. It uses Section, FieldGrid and existing controls. Opening starts with the last hour; presets and a custom UTC interval cover retained history. The last-hour view refreshes five seconds after the preceding request completes. Successful automatic refresh keeps existing readings visible without a loading status; initial loads and explicit refresh still show progress, and errors remain visible. The 24-hour, 30-day and custom intervals are fixed snapshots refreshed explicitly, avoiding repeated month-long scans. Pinning a report stops automatic refresh. Raw reports and inspection remain available if the trail fails or exceeds its point budget. Following keeps the last complete report and its readings together while the next inspection loads. Explicit Refresh discovers backfill without requiring a live feed event. Return to recent resumes following the last hour.
...
Fixed-interval raw pagination reuses the existing trail request and result; only a new interval or explicit refresh starts another trail scan. The sidebar labels intervals limited by retention on their first page, and camera movement clears hover previews. Movement timestamp parsing accepts the Protocol's lowercase notation and folds leap seconds onto the following second, matching Go/PostgreSQL storage; SDK and Radio comparisons use the same normalization. Generated movement input types preserve quantity and coordinate-pair constraints.

Trail and inspection response checks verify their named quantities, report times, order and interval bounds; report time-source flags must agree with their timestamps. Trail count/simplification metadata must match the returned points. Raw-page navigation binds its stack to the displayed cursor, so repeated clicks cannot skip or duplicate pages. SQL lower bounds round up to the next representable microsecond when needed, keeping returned samples inside a caller's nanosecond-precision interval.

## Import contention check

A local PostgreSQL 17 comparison used 20 batches of 500 reports while issuing alternating live Entity updates to the imported Asset and another Asset. The baseline was `504bacd5`. Both versions used the same disposable database and fixture; the updated implementation sends each batch as one statement instead of individual insert round trips.

| Measurement | Baseline | Bulk import |
| --- | ---: | ---: |
| 500-report import median | 283 ms | 14 ms |
| 500-report import p95 | 358 ms | 16 ms |
| Concurrent live-update p95 | 323 ms | 11 ms |
| Concurrent live-update maximum | 364 ms | 15 ms |

This reduces the Entity lock's hold time without changing live change-clock ordering or snapshot pagination. Imports still serialize against writes to the same Entity; database delays can still propagate through that contention. This is a local comparison, not a slow-storage or full-deployment guarantee.

The bulk-import test covers the maximum batch size, equivalent normalized timestamps, duplicates within a batch and across retries, preservation of first arrival time, and whole-batch rollback on conflicting IDs. Existing tests cover expiry, stored-report conflicts and concurrent snapshot pagination.

Other review claims do not currently require code changes: value-based hashing cannot distinguish live-write retries from new unchanged reports; and the existing month-trail measurement supports the bounded scan at v1 scale. HTTP 409 already becomes the SDK's typed `ConflictError` regardless of the body error code. Clock-ahead display remains a separate product decision: the current view ends at now, while accepted future-dated observations remain stored until that interval reaches them.

## ChatGPT review fixes

An Extra High review of PR #338 at `c4faf5c013666115953075834c32fcf1c1f43e36` identified three issues, reproduced locally. Imports now check conflicts across expired and retained inputs before applying the insertion cutoff. Range changes, return-to-recent, refresh and page navigation immediately abort the old raw-history request, so a response arriving before effect cleanup cannot repopulate a cleared view. Re-selecting a range also starts a fresh request. Radio history pages are capped at 100 samples, and Radio trails require an explicit budget of 2–100 points. HTTP budgets remain unchanged.

Focused tests reproduce mixed-age and stored-ID conflicts, both UI transitions before effect cleanup, and large movement responses exceeding the Radio framing limit. The Radio boundary test covers 100 reports with escaped maximum-length IDs, long finite numeric values, Core-format timestamps and cursor overhead. Envelope size and radio delivery deadlines remain transport constraints; this is not a physical-radio throughput claim.

Validation after these fixes: all Core action tests passed against disposable PostgreSQL with the race detector, plus `go vet`. The Radio package passed generation checks, formatting, lint, typechecking, all 443 tests and build. Command passed lint, formatting, typechecking, all 22 movement-history tests, production build and bundle budgets. `git diff --check` passed. All three final review findings were accepted and fixed.

## Thermos review fixes

The generated `MovementSample` output type now requires a movement quantity, paired coordinates and known fields, matching runtime validation. The canonical schema uses the same explicit quantity variants as movement inputs, and Protocol and Radio artifacts were regenerated. Type tests reproduced metadata-only, half-coordinate and misspelled-field acceptance before the fix and reject each afterward. Return to recent now calls the existing one-hour range reset, including its immediate request cancellation.

Protocol artifact checks and Go tests, 511 SDK tests in both Node and Chromium, SDK type tests, all 443 Radio tests and all 22 history UI tests pass. SDK and Command typechecks and builds pass. The explicit schema variants slightly enlarge the generated runtime validators: the measured initial JavaScript is 430.57 kB raw and the total is 3,653.48 kB. Their raw limits each increase by 1 kB. Linux CI measured the initial gzip bundle at 130.06 kB, compared with 129.70 kB on macOS, so its limit increases by 500 bytes to 130.50 kB. Other compressed-size limits remain unchanged.
