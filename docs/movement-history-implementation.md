...
Other review claims do not currently require code changes: FieldLink's integrating application owns complete Protocol validation; value-based hashing cannot distinguish live-write retries from new unchanged reports; and the existing month-trail measurement supports the bounded scan at v1 scale. HTTP 409 already becomes the SDK's typed `ConflictError` regardless of the body error code. Clock-ahead display remains a separate product decision: the current view ends at now, while accepted future-dated observations remain stored until that interval reaches them.
