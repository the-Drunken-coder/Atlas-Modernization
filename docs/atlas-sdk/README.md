...
2. **Sync engine** — optional. Local resource state + change feed consumer + reconciliation loop.

Within the sync engine, `SyncEngine` owns transport, lifecycle and recovery ordering, the single global feed cursor, and callback delivery. `ResourceCache` owns resource versions, point-read race guards, local and feed delete reconciliation, immutable values and snapshots, and the decision to publish resource or snapshot changes. It returns those decisions to `SyncEngine`; it never invokes application callbacks itself.
...
- **Gap detection:** `SyncEngine` tracks the last processed global feed version N. If an event arrives with a version that skips past expected values, the SDK immediately calls `changed-since?since_version=N` to catch up. Recovery is event-driven, not timer-driven. Resource rejection does not prevent a processed feed event from advancing this cursor.
