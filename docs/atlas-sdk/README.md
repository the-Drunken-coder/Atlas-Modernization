...
`client.runtime.begin`, `ready`, and `tasks` expose process registration, manifest publication, and runtime-scoped delivery. `begin` and `ready` carry the runtime ID in their registration request bodies; `tasks` carries it in `Atlas-Runtime-ID`. The SDK does not infer process restarts from transport reconnects.
