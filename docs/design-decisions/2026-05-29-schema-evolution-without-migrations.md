...
2. **Name:** Ephemeral runtime storage — destroy-and-recreate database and bucket on startup (no migrations)
...
7. **Location:** `Atlas_Core/internal/database/db.go` (`EnsureTables`), `Atlas_Core/internal/storage/storage.go` (`EmptyBucket`), `Atlas_Core/cmd/atlas_core/main.go` (startup order), `Atlas_Core/docs/DATABASE_WORKFLOW.md`, `Atlas_Core/internal/config/config.go` (`DATABASE_RECREATE_ON_STARTUP`)
