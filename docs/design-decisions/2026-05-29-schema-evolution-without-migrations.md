...
4. **Decision:** Update the original decision in place rather than adding a second compatibility posture.
   ...
   - Production startup ensures the configured MinIO bucket exists and never empties it. A configured-but-unavailable production bucket is startup-fatal so Core cannot serve durable object metadata without its blob store.
