...
- A handler return value becomes Task output. Returning nothing completes without output. Output that is not JSON-serializable without numeric loss fails the Task with `execution_failed`, as does a thrown error.
