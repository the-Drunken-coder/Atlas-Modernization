...
7. **Actual:** Requests still include no `X-API-Key` or bearer auth. `test_api_manual.py` recognizes only one exact configured URL as remote and uses obsolete raw status changes; both scripts swallow failures and return normally. Repository search found no maintained consumer. This was revalidated against `main` at `f4b0187fdb68088ea0b59d28218d02204f4cfc9c`.
8. **Reproduction:**
