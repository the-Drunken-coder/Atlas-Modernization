...
3. **Issue:** Inspector placeholders, formatting helpers, status descriptions, and one map comment use em dashes despite the machine-wide visual constraint prohibiting them.
4. **Severity:** S4 (Minor)
5. **Location:** Atlas Command Interface under `atlas_command_interface/src/features/`, `atlas_command_interface/src/atlas/format.ts`, `atlas_command_interface/src/ui/primitives/StatusPill.tsx`, and `atlas_command_interface/src/ui/map/interaction/map-camera.ts`
...
7. **Actual:** `rg -n "—" atlas_command_interface/src --glob '!**/*.test.*'` reports missing-value placeholders and inline status prose that use em dashes.
8. **Reproduction:**
