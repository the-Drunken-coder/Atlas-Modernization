...
Codex review of `0af18f29` found stale catalog-only display names and a distinct paired-restore ledger issue. Restored-ledger verification must use the prior retained PostgreSQL bundle without advancing rollback completion before verification. The installed-Plugin update, rollback, uninstall, and key-rotation menu controls remain intentionally deferred pending the documented mock selection; direct CLI commands implement those operations, and documentation now makes that scope explicit.
