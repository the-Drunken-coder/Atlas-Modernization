...
Codex review of `cdeba45c` found reset bypassed the supervision gate and image receipt pulls used Node architecture. Reset now checks supervision before deleting anything, with an explicit `--manual` acknowledgement for manual environments. Receipt pulls normalize and use Docker daemon architecture, including Rosetta and Docker architecture aliases.

Codex review of `4c748319` found five remaining recovery, interaction, and release defects. Supervised Plugin recreation now uses bounded Compose readiness waits; stopped deployments no longer advertise restart; independent enable/disable preserve cancellation outcomes after rollback. Release promotion fails closed on registry inspection errors, and release/catalog jobs select Node from `.nvmrc` before validation.

Codex review of `3a6e37bc` found that missing disposable Plugin overlays blocked stop and Core recovery cleanup. Project-wide `down --remove-orphans` now omits absent schema-4 overlays so Compose can remove their containers as orphans; other operations retain strict overlay requirements. Tests cover normal stop and an interrupted target, including retained durable volumes and journal state.

Codex review of `f8bb8a9d` found four release and operator defects. Candidate acceptance now probes each published image platform. Expired idempotent publication/revocation retries renew the catalog, and published verification rejects expiry. Reset removes credentials before state so a credential deletion failure preserves retry authorization. Direct and interactive Plugin status include the authenticated revocation reason.

Codex review of `e61cb4a1` found retained image repair, release dependency ordering, and expired status-history gaps. Enable/rollback can re-pull an authenticated retained image and must reproduce its receipt exactly. Plugin release tests run after workspace dependency builds. Read-only catalog inspection preserves authenticated revocation history after expiry, while mutation reads remain freshness-gated.

Codex review of `8fb2df09` found three transition and recovery gaps. Legacy bundled Plugins now block `update all` before CLI replacement and the error names the exact prior CLI disable commands. Forward recovery revalidates credentials against its selected target even after a prior target reached `credentials-durable`. Ordinary deployment Docker commands use the existing durable process-group fence so dead-owner recovery cannot overlap a surviving child.

Codex review of `346db439` found three publication and restart contract gaps. Normal active-file regeneration no longer re-pulls missing images without explicit repair; lifecycle rollback retains its authenticated repair path. Candidate acceptance executes the deployed in-container health probe on both platforms. Signing-environment guidance permits immutable Plugin tags so source-preserving publication retries remain possible after main advances.

Codex review of `8ee514d0` found five remaining operator and documentation gaps. Stop and reset can remove verified owned containers when the retained Compose file is missing. Restart requires active supervision unless explicitly acknowledged with `--manual`. Catalog expiry warnings use the same monotonic clock as admission, and the menu displays catalog errors for installed Plugins. Wire documentation now states that both private protocol-major fields are required and sent by the runtime.

Codex review of `237627e4` found two confirmed recovery gaps: read-only status must preserve authenticated cached revocation history below a newer CLI checkpoint, and missing disposable Plugin overlays must not block the supervisor before recovery. Inspection now reports checkpoint warnings while mutation admission stays strict. The separate prior-PostgreSQL-image allegation was disproved: candidate files remain staged during the ledger read, and the retained prior Compose file still supplies the PostgreSQL digest. A test with distinct prior and target digests preserves that evidence; runtime ordering is unchanged.

Codex review of `1484a1c0` found supervisor endpoint/version drift and two menu gaps. Service definitions now pin the selected local Docker socket, clear inherited context overrides, and record the CLI version. CLI upgrades refresh active matching supervision before Core updates; stale or mismatched active services require explicit reinstallation. The Plugin menu retains refresh failures alongside cached rows and offers installation for catalog-only Plugins.

Codex review of `0af18f29` found stale catalog-only display names and a distinct paired-restore ledger issue. Restored-ledger verification must use the prior retained PostgreSQL bundle without advancing rollback completion before verification. At that review point, installed-Plugin update, rollback, uninstall, and key-rotation menu controls remained deferred pending mock selection. The [2026-09-12 TUI decision](../design-decisions/2026-09-12-atlas-core-tui-redesign.md) supersedes that pending status. The approved follow-up adds ordinary one-at-a-time Plugin updates, while rollback, uninstall, catalog refresh, and shared-key rotation remain direct-command-only.
