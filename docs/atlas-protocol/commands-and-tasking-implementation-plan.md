...
The concrete Task module accepts the generated catalog as data. Production supplies the empty generated catalog; conformance tests supply a small fixture catalog from `packages/protocol/conformance/tasking/fixtures/`. This is test data, not a second production catalog or an asset extension point. The two fixture schemas remain in the canonical bundle as the narrow boundary exception documented in [the Protocol package](../../packages/protocol/README.md#boundary); fixture Commands remain absent from the production catalog.
