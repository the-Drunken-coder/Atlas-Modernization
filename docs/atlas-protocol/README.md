...
Atlas Protocol is the reusable contract layer for Atlas data. The buildable module lives in [`packages/protocol/`](../../packages/protocol/); this directory keeps the project-level design notes.

## Current Shape

- JSON Schema in `packages/protocol/schema/jsonschema/atlas.schema.json` is the source of truth.
- Checked-in examples validate against the JSON Schema source.
- Authored, schema-parity-checked Go types plus generated Go validators, TypeScript types and runtime predicates, and a protocol revision stamp live under `packages/protocol/generated/`.
...
The shared request corpus in `packages/protocol/conformance/request-validation.json` checks the canonical schema, Go validation, generated TypeScript predicates, and Core request boundaries against the same cases. `packages/protocol/conformance/tasking/` adds portable lifecycle, scheduling, outcome, and restart scenarios using fixture Commands that never enter the production catalog.
...
Use the module-level workflow in [`packages/protocol/README.md`](../../packages/protocol/README.md):
