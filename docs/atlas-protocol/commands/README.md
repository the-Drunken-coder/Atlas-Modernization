...
Atlas Protocol owns every Command. The generated production catalog currently
contains the `flight` namespace (takeoff, go-to, return-to-launch, land); a
Command becomes real only when its complete cross-module behavior lands
together.
...
3. Add the definition to `packages/protocol/commands/<namespace>.json`. The namespace file must match the prefix of every Command it contains.
...
5. Add a purpose-built operator input to `surfaces/command-interface/src/features/commands/command-input-registry.tsx`.
...
The source-side generator rules live in [`packages/protocol/commands/README.md`](../../../packages/protocol/commands/README.md). Generated catalogs and codecs are never edited directly. Test-only fixture Commands remain under `packages/protocol/conformance/tasking/fixtures/` and never enter the production catalog.
