...
Keep normal simulation work pointed at a local Atlas Core (`ATLAS_BASE_URL=http://localhost:8000`). The config also accepts HTTPS Atlas Core targets, including `ATLAS_BASE_URL=https://atlascommandapi.org`, for isolated, disposable tenants because run state is local to this workbench and cleanup can only remove resources while the run remains in memory. Plain HTTP is only accepted for loopback development URLs.
