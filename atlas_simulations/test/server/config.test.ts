...
describe("loadConfig", () => {
  ...
  it("rejects invalid ATLAS_BASE_URL values", () => {
    ...
    expect(() => loadConfig({ env: { ATLAS_BASE_URL: "https://atlas.test?bad=true" }, packageRoot })).toThrow(
      "ATLAS_BASE_URL must not include a query string or fragment"
    );
    expect(() => loadConfig({ env: { ATLAS_BASE_URL: "https://atlas.test/#bad" }, packageRoot })).toThrow(
      "ATLAS_BASE_URL must not include a query string or fragment"
    );
  });
  ...
  it("builds a deployed target only when explicitly enabled with a URL", () => {
    ...
    const config = loadConfig({
      env: {
        ...
        ATLAS_API_KEY: "legacy-local-key",
  ...
  it("treats blank env overrides as unset", () => {
    ...
    expect(config.atlasBaseUrl).toBe("http://127.0.0.1:8000/api");
  });
