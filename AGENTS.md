...
- This is a multi-module repository. `atlas_core/` and `atlas_protocol/` are separate Go modules; the SDK, asset runtime, command interface, and simulations are npm workspaces sharing the root lockfile. Use Node 24 from the root `.nvmrc`, install from the repository root, and consume workspace packages through their public exports.
- Atlas Protocol's source of truth is `atlas_protocol/schema/jsonschema/atlas.schema.json`. The authored Go API remains in `atlas_protocol/generated/go/atlasprotocol/types.go`; regenerate artifacts with `go run ./tools/generate` from `atlas_protocol/`. Planning and reference docs belong in `docs/atlas-protocol/`, never a root `Atlas Protocol/` directory.
...
- The command interface is a static Cloudflare Pages/Vite app whose browser SDK calls Atlas Core directly. Do not recreate the retired Worker proxy or its runtime config/auth/API routes; see `atlas_command_interface/README.md`.

## Safety

- Production Atlas Core storage is durable; development Compose is intentionally destructive scratch storage. Never run scratch mode or an older destructive stack against retained data. Follow `atlas_core/docs/DATABASE_WORKFLOW.md` and `atlas_core/docs/DEPLOYMENT_RUNBOOK.md`.
- Simulations target loopback Core by default. Deployed runs require the explicit safeguards documented in `atlas_simulations/README.md`; never bypass their confirmation, target identity, credential, or cleanup-ledger rules.
