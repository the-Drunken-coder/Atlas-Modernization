...
- This is a multi-module repository. `services/core/` and `packages/protocol/` are separate Go modules; the SDK, Meshtastic Link, command interface, and simulations are npm workspaces sharing the root lockfile. Use Node 24 from the root `.nvmrc`, install from the repository root, and consume workspace packages through their public exports.
