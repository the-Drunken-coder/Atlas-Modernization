...
**Trust order.** Sections 1–6 are durable: they describe architecture and intent that changes only by deliberate decision. Section 7 is perishable — it pins behavior of specific tool versions and goes stale silently. If current code, tests, or docs contradict this file, verify the source of truth before coding around stale guidance, then tell the developer what was stale.
...
Keep the active development/CI Node version in the root `.nvmrc`; the command-interface `.nvmrc` mirrors it for tooling that inspects the package directory. The root `.nvmrc` and package engines are the Node version source of truth.
...
Coverage is gated two ways per JavaScript workspace. Vitest `thresholds` enforce package-wide floors, and `scripts/check-file-coverage.mjs` enforces a per-file statement floor. Both matter: Vitest thresholds — including its glob entries — are scored over the *combined total* of the files they match, so a glob floor cannot catch one new uncovered file. Glob entries in the configs behave per-file only because each pattern names exactly one file. Files that cannot meet the per-file floor yet are pinned at their current value in that script's `EXEMPTIONS`; lower those as tests land, never raise one to make a regression pass.
...
Admin session cookies default to `SameSite=none`, so CSRF is defended by a strict Origin allowlist rather than by the cookie attribute. `requireTrustedAdminOrigin` is applied per handler, not as middleware — when adding any state-changing `/admin/*` route, add that check explicitly or the route is open to cross-site requests.
...
## 6. SDK, command interface, and simulations
...
The sync engine's failure state is a `SyncErrorCode`, and its health is one `SyncHealth` value. Branch on those, never on the display text in `SYNC_ERROR_MESSAGES` — the strings are presentation only and are safe to reword. `SyncStatus` still reports `healthy`/`degraded` as separate booleans for consumers; derive them, do not reintroduce paired boolean fields.
...
The asset-runtime migration is an API proving ground, not an extraction of an existing generic loop: current simulations directly performed check-ins and did not contain repeated handshake, reconnect, task-dispatch, or feed-consumption machinery. Do not preserve or claim behavior that was never there.

### Do not copy audit-era shortcuts forward

These apply when working on Atlas Core, Atlas Protocol, the SDK, or the command interface.
...
## 7. Toolchain gotchas (perishable — pinned to specific tool versions)

Entries here describe the behavior of one version of one tool. Re-verify before trusting any of them; delete an entry once the pinned version is gone.

**Node / npm.** `.nvmrc` and package engines currently require Node 24, despite stale docs that previously said Node 26. For the packed CLI smoke under npm 11, assert that installation created `node_modules/.bin/atlas`, then run the installed `bin.atlas` module with Node. Alias-only `npx --no-install atlas ...` and `npm exec -- atlas ...` invocations are rejected as unsupported `npm exec` usage in this harness.

**Biome 2.5.5.** Type-aware `noFloatingPromises` and `noMisusedPromises` are enabled for all four JavaScript workspaces through package-local configurations with the `types` domain set to `recommended`. Keep each lint script's explicit `--config-path biome.json`: without it, Biome resolves the repository-root configuration and silently skips the package-local type-aware rules. Biome 2.5.3's namespace-import workaround no longer prevents its module resolver from panicking with the current dependency layout; keep all workspaces aligned on 2.5.5 or newer rather than adding lint exemptions, and keep the root `biome.json` `$schema` on the same version as the workspace configs. Relatedly, `src/ui/map/interaction/map-reticle-interaction-types.ts` exists to hold the options type shared by the reticle orchestrator, pointer, and effects modules — folding it into any of them creates an import cycle in that same directory. Keep it.

**Wrangler.** `wrangler pages dev` can select today's compatibility date even when that date is newer than its bundled `workerd` runtime. Keep the explicit supported `compatibility_date` in `atlas_command_interface/wrangler.jsonc` so local Pages and `_headers` validation starts deterministically.

**Git worktrees on macOS.** Codex-created worktrees may not be checked out on the PR branch even when they are inside this repository. If the local tree looks unexpectedly small or detached, run `git worktree list` and inspect the branch checkout before deciding the PR contents are missing. On case-insensitive macOS filesystems, an older worktree can physically retain the historical `Atlas_Core` spelling even though Git canonically tracks `atlas_core/`. Normalize it through an explicit temporary directory name and verify `git status` remains clean; never commit or recreate the uppercase path.
