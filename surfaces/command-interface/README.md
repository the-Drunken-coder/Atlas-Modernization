...
Command submission posts `{ asset_id, command, input }` directly to Core with a fresh idempotency key for that tasking attempt. Core validates the generated catalog, current ready runtime manifest, and input, then generates the Task ID. The production catalog currently contains the `flight.*` Commands; an Asset still shows no Commands until its ready runtime advertises them and a purpose-built input is registered.
...
Map vertex handles support pointer dragging and keyboard editing. Arrow keys move the focused vertex 10 screen pixels; Shift+Arrow moves it 40 pixels. Delete removes the vertex when the geometry allows removal. Focus stays on the edited vertex after movement. The inspector also provides coordinate fields and vertex controls.

Plugin **Draw area** temporarily hides geometry edit handles and the edit overlay. The draft stays in memory, and the handles return when drawing ends. Return to the geofeature inspector to save or cancel the draft.
...
In Geo Features, select **Add Geo Feature**, enter a name, and choose Point, Line, Polygon, or Circle. Click the map to place a point or circle center. For lines and polygons, click each vertex, then select **Finish drawing** or click the first vertex to close a polygon. Dashed edges preview the next vertex without saving it. **Undo** or Backspace removes the last vertex while drawing; Backspace in text fields edits text normally. Circles start with a 100 m radius that can be changed before saving. Adjust coordinates or drag the map handles, then select **Create feature**. Names are stored as Core entity aliases and must be unique.

Drawing owns map clicks until finished or cancelled. Redraw clears the current geometry. Cancel, Back, or switching tabs asks before discarding a named draft or one with geometry; saving temporarily prevents leaving the editor. Failed requests keep the draft and its entity ID for retry. Successful creation opens the existing inspector. Create and Cancel stay visible while the coordinate list scrolls. On narrow screens, the map appears above the creation editor.

### Selected Entity movement history

Asset and Track inspectors contain Movement History below Location & Movement. It reads 30-day position, speed and altitude history through the SDK, separately from live state. Recent history follows; selecting a past report pins its readings while current markers remain live. Dotted connectors indicate gaps longer than 60 seconds between original position reports. Controls, historical readings and the legend stay in the existing sidebar. See [behavior, API limits and validation](../../docs/movement-history-implementation.md).
