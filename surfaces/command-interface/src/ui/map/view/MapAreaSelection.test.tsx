...
describe("MapAreaSelection", () => {
  it("publishes the viewport and draws a map area with pointer input", async () => {
    ...
    await waitFor(() =>
      expect(spatial.onViewportArea).toHaveBeenCalledWith({ west: -20, south: -10, east: 20, north: 10 })
    );
    ...
    fireEvent.pointerDown(canvas, { button: 0, pointerId: 7, clientX: 60, clientY: 70 });
    ...
    expect(spatial.onAreaChange).toHaveBeenLastCalledWith({ west: 50, south: 50, east: 140, north: 120 });
    ...
    await waitFor(() => expect(canvas).toHaveClass("map-canvas--region-drawing"));
    fireEvent.pointerDown(canvas, { button: 0, pointerId: 8, clientX: 60, clientY: 70 });
    ...
    rerenderMap({ spatial: { ...spatial, drawing: true } });
    await waitFor(() => expect(canvas).toHaveClass("map-canvas--region-drawing"));
    fireEvent.pointerDown(canvas, { button: 0, pointerId: 9, clientX: 80, clientY: 90 });
  ...
  it("moves and resizes an existing area with pointer and keyboard input", async () => {
    ...
    const resizeWidth = screen.getByRole("button", { name: "Resize selected area width" });
    const resizeHeight = screen.getByRole("button", { name: "Resize selected area height" });
    const resizeBoth = screen.getByRole("button", { name: "Resize selected area width and height" });

    fireEvent.keyDown(move, { key: "ArrowRight" });
    expect(spatial.onAreaChange).toHaveBeenLastCalledWith({ west: 50, south: 50, east: 150, north: 120 });
    fireEvent.keyDown(resizeWidth, { key: "ArrowRight", shiftKey: true });
    expect(spatial.onAreaChange).toHaveBeenLastCalledWith({ west: 40, south: 50, east: 180, north: 120 });
    fireEvent.keyDown(resizeHeight, { key: "ArrowDown" });
    expect(spatial.onAreaChange).toHaveBeenLastCalledWith({ west: 40, south: 50, east: 140, north: 130 });
    fireEvent.keyDown(resizeBoth, { key: "Enter" });
    expect(spatial.onAreaChange).toHaveBeenCalledTimes(3);

    fireEvent.pointerDown(move, { button: 0, pointerId: 11, clientX: 50, clientY: 60 });
    fireEvent.pointerMove(window, { pointerId: 11, clientX: 80, clientY: 100 });
    expect(spatial.onAreaChange).toHaveBeenLastCalledWith({ west: 70, south: 90, east: 170, north: 160 });
    fireEvent.pointerUp(canvas, { pointerId: 11, clientX: 80, clientY: 100 });

    fireEvent.pointerDown(resizeBoth, { button: 0, pointerId: 12, clientX: 140, clientY: 120 });
    fireEvent.pointerMove(window, { pointerId: 12, clientX: 110, clientY: 90 });
    expect(spatial.onAreaChange).toHaveBeenLastCalledWith({ west: 40, south: 50, east: 110, north: 90 });
