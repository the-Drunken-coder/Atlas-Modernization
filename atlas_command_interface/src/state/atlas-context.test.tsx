...
describe("AtlasProvider", () => {
  it("starts live sync before reading the displayed post-start snapshot", async () => {
    const calls: string[] = [];
    let currentSnapshot: AtlasSnapshot = { entities: {}, tasks: {} };
  ...
  it("refreshes rendered state from the snapshot notification after writes", async () => {
    let notify: (() => void) | undefined;
    let currentSnapshot: AtlasSnapshot = { entities: { "asset-1": entity("Initial", 1) }, tasks: {} };
    const updateGeometry = vi.fn(async (_entityId: string, _geometry: unknown, _ifMatchVersion?: number) => entity("Stale Action", 1));
    const fake: AtlasDataSource = {
      snapshot() {
        return currentSnapshot;
      },
      ...
      async updateGeometry(entityId, geometry, ifMatchVersion) {
        const result = await updateGeometry(entityId, geometry, ifMatchVersion);
        currentSnapshot = { entities: { "asset-1": entity("Cache Update", 2) }, tasks: {} };
        notify?.();
        return result;
      },
      dispose() {}
    ...
    await waitFor(() => expect(screen.getByTestId("entity-names")).toHaveTextContent("Cache Update"));
  });
});
