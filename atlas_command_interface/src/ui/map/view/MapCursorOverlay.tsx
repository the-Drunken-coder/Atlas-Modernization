...
import { type CursorOverlayState, formatImperialDistance, tetherSegment } from "../interaction/map-cursor-overlay.js";

export function MapCursorOverlay({
  point,
  coordinates,
  selection,
  distanceMeters,
    ...
    <>
      {selection ? (
        <svg className="map-cursor-locator" aria-hidden="true">
          {tether ? (
            <line
              className="map-cursor-locator__tether"
              x1={tether.start.x}
              y1={tether.start.y}
              x2={tether.end.x}
              y2={tether.end.y}
