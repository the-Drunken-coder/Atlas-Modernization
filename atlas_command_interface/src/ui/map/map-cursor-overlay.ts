...
export function tetherSegment(selection: TargetBox, pointer: ScreenPoint): { start: ScreenPoint; end: ScreenPoint } | null {
  if (pointer.x >= selection.x && pointer.x <= selection.x + selection.width && pointer.y >= selection.y && pointer.y <= selection.y + selection.height) {
    return null;
  }
  ...
  const scaleToEdge = Math.min(dx === 0 ? Infinity : selection.width / 2 / Math.abs(dx), dy === 0 ? Infinity : selection.height / 2 / Math.abs(dy));
...
export function formatImperialDistance(distanceMeters: number): string {
  ...
  return feet < FEET_PER_MILE ? `${Math.round(feet).toLocaleString("en-US")} ft` : `${(feet / FEET_PER_MILE).toFixed(2)} mi`;
}
