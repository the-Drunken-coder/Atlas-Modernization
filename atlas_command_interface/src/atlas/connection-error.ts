...
const SENSITIVE_FIELD = /(\b(?:api[_-]?key|access[_-]?token|refresh[_-]?token|authorization|cookie|password|secret|token)\b\s*[:=]\s*)(?:"[^"]*"|'[^']*'|[^;\n\r}]+)/gi;
...
export function sanitizeConnectionError(cause: unknown): string {
  ...
  const sanitized = firstLine.replace(SENSITIVE_PARAMETER, "$1[redacted]").replace(SENSITIVE_FIELD, "$1[redacted]").replace(KNOWN_SECRET, "[redacted]");
