...
2. Start local Atlas Core from this checkout:
   ...
   `atlas.py` starts Docker Compose, waits for PostgreSQL, MinIO, and the API, then publishes the embedded command catalog through the object API for browser/reference use. Startup seeds the `admin` account with the generated `ATLAS_ADMIN_PASSWORD` stored in the owner-only `Atlas_Core/docker/.env`.
...
Open http://127.0.0.1:5173/map and sign in as `admin` with the `ATLAS_ADMIN_PASSWORD` from `Atlas_Core/docker/.env`.
