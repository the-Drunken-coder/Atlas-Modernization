...
func ValidateObjectBlob(value any) []string {
	...
	if sizeBytes, exists := blob["size_bytes"]; exists {
		errors = append(errors, validateNonNegativeInteger(sizeBytes, "object.size_bytes")...)
	}
...
func validateNonNegativeInteger(value any, path string) []string {
	num, ok := integerFromValue(value)
	if !ok {
		return []string{path + ": expected non-negative integer"}
	}
	if num < 0 {
		return []string{fmt.Sprintf("%s: %d must be non-negative", path, num)}
	}
	return nil
}
...
func integerFromValue(value any) (int64, bool) {
	...
	case uint32:
		return int64(typed), true
	case uint64:
		if typed > maxInt64Value {
			return 0, false
		}
		return int64(typed), true
	case float32:
		return integerFromFloat(float64(typed))
	case float64:
...
func integerFromFloat(value float64) (int64, bool) {
	...
	if math.Trunc(value) != value || value < minInt64Float || value >= maxInt64PlusOneFloat {
		return 0, false
	}
...
const (
	...
	minInt64Float        = -float64(uint64(1) << 63)
	maxInt64PlusOneFloat = float64(uint64(1) << 63)
)
