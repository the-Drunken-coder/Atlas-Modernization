...
func runtimeValidatorHelpersSource() string {
	...
	builder.WriteString("  if (!atlasProtocolIsRecord(value) || !Array.isArray(value[\"commands\"])) return false;\n")
	builder.WriteString("  const commandIDs = new Set<string>();\n")
	builder.WriteString("  for (const command of value[\"commands\"]) {\n")
	builder.WriteString("    if (!atlasProtocolIsRecord(command) || typeof command[\"id\"] !== \"string\" || !atlasProtocolIsRecord(command[\"parameters_schema\"])) return false;\n")
	builder.WriteString("    if (commandIDs.has(command[\"id\"])) return false;\n")
	builder.WriteString("    commandIDs.add(command[\"id\"]);\n")
	builder.WriteString("    for (const parameter of Object.values(command[\"parameters_schema\"])) {\n")
	builder.WriteString("      if (!atlasProtocolIsRecord(parameter)) return false;\n")
	builder.WriteString("      const hasMinimum = atlasProtocolHasOwn(parameter, \"minimum\");\n")
	builder.WriteString("      const hasMaximum = atlasProtocolHasOwn(parameter, \"maximum\");\n")
	builder.WriteString("      if (parameter[\"type\"] !== \"number\" && (hasMinimum || hasMaximum)) return false;\n")
	builder.WriteString("      if (hasMinimum && hasMaximum && (parameter[\"minimum\"] as number) > (parameter[\"maximum\"] as number)) return false;\n")
	builder.WriteString("    }\n")
	builder.WriteString("  }\n")
	builder.WriteString("  return true;\n")
	builder.WriteString("}\n\n")
	...
	builder.WriteString("      continue;\n")
	builder.WriteString("    }\n")
	builder.WriteString("    if (typeof current !== \"object\" || (!Array.isArray(current) && !atlasProtocolIsRecord(current))) {\n")
	builder.WriteString("      return false;\n")
	builder.WriteString("    }\n")
