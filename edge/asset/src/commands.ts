...
/** Guided takeoff to an absolute altitude (meters above mean sea level). */
...
const GOTO_TYPE_MASK = 0b0000_1000_1111_1000;
// Ignore velocity, acceleration, yaw, and yaw rate: position + altitude only.
const HOLD_TYPE_MASK = 0b0000_1000_1111_1000;
...
export function gotoPositionCommand(
  ...
  return positionTarget(targetSystem, targetComponent, latitudeDeg, longitudeDeg, altitudeMslM, GOTO_TYPE_MASK);
}
...
export function holdPositionCommand(
  ...
  return positionTarget(targetSystem, targetComponent, latitudeDeg, longitudeDeg, altitudeMslM, HOLD_TYPE_MASK);
}
