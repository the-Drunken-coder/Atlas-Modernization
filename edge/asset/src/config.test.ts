...
describe("parseAssetConfig", () => {
  ...
  it("loads the credential-free example configuration", async () => {
    const examplePath = join(dirname(fileURLToPath(import.meta.url)), "..", "config.example.toml");
    const text = await readFile(examplePath, "utf8");
    const config = parseAssetConfig(text);
    expect(config.assetId).toBe("quad-01");
    expect(config.link.transport).toBe("serial");
    expect(config.apiKey).toContain("PASTE");
  });

  it("returns a plain value object with no live-reload surface", () => {
    const config = parseAssetConfig(VALID);
    expect(Object.getPrototypeOf(config)).toBe(Object.prototype);
    expect(typeof (config as { reload?: unknown }).reload).toBe("undefined");
    expect(typeof (config as { watch?: unknown }).watch).toBe("undefined");
    // Mutating the returned object never reaches a shared live source: a second
    // parse yields the original values again.
    (config as { assetId: string }).assetId = "mutated";
    expect(parseAssetConfig(VALID).assetId).toBe("quad-01");
  });
