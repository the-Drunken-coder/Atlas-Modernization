...
export class CoreAttachment {
  ...
  async checkin(telemetry: FlightTelemetry): Promise<void> {
    ...
    const flight: { armed?: boolean; flight_mode?: string; launch_elevation_m?: number } = {};
    ...
    if (telemetry.launchElevationM !== undefined) flight.launch_elevation_m = telemetry.launchElevationM;
    if (Object.keys(flight).length > 0) {
      await this.client.entities.checkIn(this.assetId, {
        telemetry: flat,
        components: { telemetry: flight }
      });
    } else {
