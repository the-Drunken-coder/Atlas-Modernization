...
describe("flight command encoding", () => {
  it("encodes takeoff with the absolute MSL altitude", () => {
    const decoded = roundTrip(takeoffCommand(1, 1, 585.5), common.CommandLong);
    expect(decoded.command).toBe(common.MavCmd.NAV_TAKEOFF);
    expect(decoded._param7).toBeCloseTo(585.5, 5);
