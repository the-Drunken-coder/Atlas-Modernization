...
| Entity response type alias | Responses currently emit canonical `entity_type` plus duplicate `type` | Legacy/duplicate response alias | Canonical Protocol field is `entity_type`; remove duplicate `type` when Protocol takes over. |
...
- Entity response duplicate `type` -> use `entity_type`.
