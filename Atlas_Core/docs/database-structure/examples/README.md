...
**API vs storage:** HTTP create/update uses `entity_type` in the request body; the database column is `type`. API responses expose `entity_type`, with metadata under `metadata.created_at`, `metadata.updated_at`, and `metadata.version`.
