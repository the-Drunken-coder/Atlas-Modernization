...
def main() -> int:
    profile = Path(sys.argv[1] if len(sys.argv) > 1 else "coverage.out")
    ...
    for name, (covered, total) in coverage(profile).items():
        observed = displayed_percent(covered, total)
        floor = FLOORS[name]
        print(f"{name}: {observed:.1f}% ({covered}/{total}), floor {floor:.1f}%")
        if observed < floor:
            failed = True
