...
// AssertDetailsContain fails the test unless one of the validation error
// details matches want (using the fuzzy CUE-message matching of MessageContains).
func AssertDetailsContain(t *testing.T, details []string, want string) {
	t.Helper()
	for _, d := range details {
		if MessageContains(d, want) {
			return
		}
	}
...
// MessageContains reports whether a protocol validation message matches the
// expected human-readable description, tolerating CUE error formatting.
func MessageContains(actual, want string) bool {
